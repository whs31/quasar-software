...
namespace GUI
{
  ...
  void VT100Terminal::newline() noexcept
  {
  ...
  void VT100Terminal::print(const QByteArray& data) noexcept
  {
    QString text;
...
//                    _curentFormat = _format;
  ...
  QString VT100Terminal::lastLine() const noexcept
  {
    if(storage.empty())
      return {};
    return storage.back().msg;
  }
  void VT100Terminal::setLastLine(const QString& str)
  {
    if(storage.size() == 0)
      return;

    storage[storage.size() - 1].msg = str;
    emit dataChanged(createIndex(rowCount() - 1, 0), createIndex(rowCount() - 1, 0));
  }

  void VT100Terminal::appendString(const QString& text)
  {
    auto str = lastLine();
    ...
    if(storage.empty() and removeCount != 6)
      this->append(str);
    else
