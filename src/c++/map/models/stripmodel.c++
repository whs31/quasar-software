...
namespace Map
{
    ...
    if(data(index, role) != value)
    {
      switch(role)
      {
        ...
        case Filename: storage[index.row()].filename = value.toString();
          break;
        case Latitude: storage[index.row()].coordinate.setLatitude(value.toDouble());
          break;
        case Longitude: storage[index.row()].coordinate.setLongitude(value.toDouble());
          break;
        case DX: storage[index.row()].dx = value.toFloat();
          break;
        case Azimuth: storage[index.row()].azimuth = value.toFloat();
          break;
        case LX: storage[index.row()].lx = value.toFloat();
          break;
        case LY: storage[index.row()].ly = value.toFloat();
          break;
  ...
  void StripModel::add(const StripImage& image)
  {
    ...
    qDebug() << "[STRIPMODEL] Model received new image. Now it contains" << rowCount() << "images";
  }
  ...
  void StripModel::remove(int index)
  {
    ...
    qDebug() << "[STRIPMODEL] Model lost an image. Now it contains" << rowCount() << "images";
  }
  ...
  void StripModel::clear()
  {
    ...
    qDebug() << "[STRIPMODEL] Model cleared";
  }
  ...
  bool StripModel::exportSelectedImages(const QString& target) noexcept
  {
    ...
    if(not res.empty())
    {
      qInfo() << "[STRIPMODEL] Marked" << res.size() << "images for export. Exporting...";
      emit markedForExport(res, target);
      return true;
    }
    else
    {
      qWarning() << "[STRIPMODEL] No images marked for export";
      return false;
    }
  }
