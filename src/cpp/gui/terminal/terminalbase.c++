...
void TerminalBase::replaceLast(const QString &message, const QString &module, MessageType type)
{
    storage.back().msg = message;
    storage.back().mdl = module;
    storage.back().type = type;

    emit dataChanged(index(rowCount() - 1), index(rowCount() - 1), { Message, Module, Type });
}
